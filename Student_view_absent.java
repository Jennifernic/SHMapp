...
public class Student_view_absent extends AppCompatActivity {
    SharedPreferences sh;
    String method = "";
    String namespace = "http://dbcon/";
    String soapAction = "";
    String[] event, a_status, a_date,val;
    ListView l;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_student_view_absent);
        sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        l = (ListView) findViewById(R.id.lv671);

        try {
            Toast.makeText(getApplicationContext(), "Haiiii", Toast.LENGTH_LONG).show();
            method = "student_view_absent";
            soapAction = namespace + method;
            SoapObject sop = new SoapObject(namespace, method);
            sop.addProperty("logid", sh.getString("logid", ""));
//            sop.addProperty("request_id",Student_view_request.rids);
    ...
    public void inItArraySize(int len) {
        ...
        a_status = new String[len];
        a_date = new String[len];
        val = new String[len];
    }
